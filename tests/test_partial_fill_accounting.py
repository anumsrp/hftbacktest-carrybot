...
def _events(*, side: str, with_trade: bool, with_cross: bool):
    ...
    for idx, row in enumerate(rows): out[idx] = row
    return out
...
def _run(*, side: str, qty: float, with_trade: bool, with_cross: bool):
    ...
    asset = (hbt.BacktestAsset().add_data(_events(side=side, with_trade=with_trade, with_cross=with_cross)).linear_asset(1.0).constant_latency(1_000_000, 1_000_000).power_prob_queue_model(2).partial_fill_exchange().trading_value_fee_model(0.001, 0.002).tick_size(0.5).lot_size(1.0))
    engine = hbt.HashMapMarketDepthBacktest([asset])
    order_id = 1
    try:
        engine.elapse(N + 1)
        if side == "buy": engine.submit_buy_order(0, order_id, 100.0, qty, hbt.GTC, hbt.LIMIT, False)
        else: engine.submit_sell_order(0, order_id, 100.5, qty, hbt.GTC, hbt.LIMIT, False)
        engine.wait_order_response(0, order_id, 2 * N)
        engine.elapse(9 * N)
        order = engine.orders(0).get(order_id)
        state = engine.state_values(0)
        return {"status": int(order.status), "exec_qty": float(order.exec_qty), "leaves_qty": float(order.leaves_qty), "position": float(engine.position(0)), "fee": float(state.fee), "num_trades": int(state.num_trades), "volume": float(state.trading_volume)}
    finally:
        engine.close()
...
def test_partial_fill_only_updates_accounting(side, expected):
    ...
    assert r["volume"] == pytest.approx(5.0 * px)
...
def test_partial_then_final_accumulates_each_execution_once(side, expected):
    ...
    assert r["volume"] == pytest.approx(20.0 * px)
...
def test_l3_builder_is_explicitly_not_applicable():
    asset = (hbt.BacktestAsset().add_data(_events(side="buy", with_trade=True, with_cross=False)).linear_asset(1.0).constant_latency(1_000_000, 1_000_000).l3_fifo_queue_model().partial_fill_exchange().trading_value_fee_model(0.001, 0.002).tick_size(0.5).lot_size(1.0))
